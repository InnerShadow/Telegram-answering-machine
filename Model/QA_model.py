...
def QA_model_train(name, model, X, Y, tokenizer, batch_size, epochs, sequences_len, maxWordsCount, messages_per_pack):
    ...
    #Create progress bar
    total_iterations = epochs * int(len(X) / messages_per_pack)
    with tqdm(total = total_iterations, desc = 'Model training progress') as pbar:
        ...
        for i in range(epochs):
            ...
            for j in range(int(len(X) / messages_per_pack)):
                ...
                history = model.fit([data_X, data_X, data_cotext], Y_categorical, batch_size = batch_size, verbose = 0)
